 ...
 *    window.show();
 */
class GLOBAL_EXPORT TopWindow : public QWidget {
    Q_OBJECT
...
public:
    ...
    explicit TopWindow(QWidget *centralWidget,
                       ...
                       const QString  &borderImagePath    = QString("://images/shadow.png"),
    ...
    /**
     * @brief 设置是否可以使窗口移动，默认是可以移动窗口的
     * @param movable 为 true 时可以移动窗口， 为 false 时不能移动
     */
      ...
      * @brief 是否需要绘制背景
      */
	 bool needDrawBG() const { return m_needDrawBG; }
	 void setNeedDrawBG(bool val) { m_needDrawBG = val; }
 signals:
     /**
      * @brief 即将关闭的信号
      */
