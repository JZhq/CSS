...
void TopWindow::setMovable(bool movable)
{
    d->movable = movable;
}
...
void TopWindow::blockShow()
{
    // 窗口在最前显示
//    this->setWindowFlags(this->windowFlags() | Qt::WindowStaysOnTopHint);
    UiUtil::getInstance()->centerWindow(this);
    show();
...
void TopWindow::paintEvent(QPaintEvent *event) {
	...
	if (needDrawBG()) //为了解决登录框在做旋转动画时，背景有不透明层的瑕疵
	{
